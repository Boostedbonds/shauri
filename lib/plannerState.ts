...
/* ── Normalize: advance current_day past any completed days ── */
function normalizeState(state: PlannerState): PlannerState {
  ...
  // Advance current_day past completed days
  while (
    next.current_day <= TOTAL_DAYS &&
    next.completed_days.includes(next.current_day)
  ) {
    next.current_day++;
  }

  // Handle cycle wrap
  if (next.current_day > TOTAL_DAYS) {
    next = {
      ...next,
      cycle:          next.cycle + 1,
      current_day:    1,
...
/* ── markComplete: add to completed, advance current_day ── */
export function markComplete(
  stateOrDay: PlannerState | number,
  maybeDay?: number
): PlannerState {
  const state = typeof stateOrDay === "number"
    ? getPlannerState()
    : normalizeState(stateOrDay);
  const day = typeof stateOrDay === "number" ? stateOrDay : maybeDay;
  if (!day) return setPlannerState(state);

  const newCompleted = uniq([...state.completed_days, day]);
  const newSkipped   = state.skipped_days.filter((d) => d !== day);

  // Find next non-completed day
  let nextDay = state.current_day;
  const completedSet = new Set(newCompleted);
  while (nextDay <= TOTAL_DAYS && completedSet.has(nextDay)) {
    nextDay++;
  }

  const updated: PlannerState = {
    ...state,
    current_day:    nextDay > TOTAL_DAYS ? state.current_day : nextDay,
    completed_days: newCompleted,
    skipped_days:   newSkipped,
    last_action: {
      type: "complete",
      day,
      cycle: state.cycle,
...
/* ── markSkipped: add to skipped, advance current_day ── */
export function markSkipped(
  ...
  const newSkipped   = uniq([...state.skipped_days, day]);
  const newCompleted = state.completed_days.filter((d) => d !== day);

  // Advance current_day to next non-completed, non-current day
  // (skipped days stay in pending, so current_day should move forward)
  let nextDay = state.current_day;
  if (nextDay === day) {
    nextDay++;
    const completedSet = new Set(newCompleted);
