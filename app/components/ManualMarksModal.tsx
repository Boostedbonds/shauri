 ...
 *
 * Drop this into your planner page.
 * Replaces the bare score/total input with a verified flow:
 *   1. Student enters marks
 *   2. Must upload question paper + answer sheet PDF/image
 *   3. AI cross-checks and either confirms or corrects
 *   4. Saves to Supabase with score_source = 'manual_verified'
 *      and error_topics populated
 */
"use client";
...
export default function ManualMarksModal({ subject, chapter, day, onSaved, onClose }: Props) {
  ...
  const [qpFile,      setQpFile]     = useState<string | null>(null);   // base64
  const [asFile,      setAsFile]     = useState<string | null>(null);   // base64
  ...
  async function verify() {
    ...
    const m = parseInt(marks), t = parseInt(total);

    try {
      const prompt = [
        `Subject: ${subject}. Chapter: ${chapter}. Day ${day}.`,
        `Student says they scored ${m} out of ${t}.`,
        `I am giving you the question paper and their answer sheet.`,
        `Please:`,
        `1. Check if the claimed score of ${m}/${t} is accurate.`,
      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          subject,
          student: JSON.parse(localStorage.getItem("shauri_student") || "{}"),
          history: [],
        }),
      });
  ...
  const btnGhost: React.CSSProperties = { ...btn, background: "transparent", color: "#2563eb", border: "1.5px solid #2563eb" };
      ...
      <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 580, maxHeight: "92vh", overflow: "auto", padding: "28px 24px 40px" }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
          {(["entry","upload","verifying","result"] as Step[]).map((s, i) => (
            <div key={s} style={{ flex: 1, height: 4, borderRadius: 2, background: (["entry","upload","verifying","result"].indexOf(step) >= i) ? "#2563eb" : "#e2e8f0", transition: "background 0.3s" }} />
          ))}
        </div>
        ...
        {step === "entry" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <button style={{ ...btn, opacity: (!marks || !total || parseInt(marks) > parseInt(total)) ? 0.5 : 1 }}
        ...
        {step === "upload" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div onClick={() => qpRef.current?.click()} style={{ border: `2px dashed ${qpFile ? "#2563eb" : "#e2e8f0"}`, borderRadius: 12, padding: 20, cursor: "pointer", background: qpFile ? "#eff6ff" : "#f8fafc", textAlign: "center" }}>
              {qpFile ? (
                <div style={{ fontSize: 14, color: "#2563eb", fontWeight: 600 }}>✓ {qpName} <span style={{ color: "#94a3b8", fontWeight: 400, fontSize: 12 }}>(tap to replace)</span></div>
              ) : (
                <>
            ...
            <input ref={qpRef} type="file" accept="image/*,application/pdf" hidden onChange={e => { const f = e.target.files?.[0]; if (f) handleQP(f); }} />

            {/* Answer sheet upload */}
            <div onClick={() => asRef.current?.click()} style={{ border: `2px dashed ${asFile ? "#059669" : "#e2e8f0"}`, borderRadius: 12, padding: 20, cursor: "pointer", background: asFile ? "#f0fdf4" : "#f8fafc", textAlign: "center" }}>
              {asFile ? (
                <div style={{ fontSize: 14, color: "#059669", fontWeight: 600 }}>✓ {asName} <span style={{ color: "#94a3b8", fontWeight: 400, fontSize: 12 }}>(tap to replace)</span></div>
              ) : (
                <>
                  <div style={{ fontSize: 28, marginBottom: 6 }}>📝</div>
            ...
            <input ref={asRef} type="file" accept="image/*,application/pdf" hidden onChange={e => { const f = e.target.files?.[0]; if (f) handleAS(f); }} />

            {error && <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, padding: "10px 14px", color: "#b91c1c", fontSize: 13 }}>⚠ {error}</div>}

            <div style={{ display: "flex", gap: 10 }}>
              <button style={{ ...btnGhost, flex: 0 }} onClick={() => setStep("entry")}>← Back</button>
              <button style={{ ...btn, flex: 1, opacity: canVerify() ? 1 : 0.5 }} disabled={!canVerify()} onClick={verify}>
                ⚡ Verify with AI
              </button>
            </div>
          </div>
        ...
        {step === "verifying" && (
          <div style={{ textAlign: "center", padding: "40px 0" }}>
            ...
            <p style={{ fontSize: 13, color: "#64748b" }}>Reading question paper and answer sheet against your claimed score of {marks}/{total}</p>
          </div>
        )}
        ...
        {step === "result" && aiResult && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {/* Score result */}
            <div style={{ background: aiResult.scoreChanged ? "#fef9c3" : "#f0fdf4", border: `1px solid ${aiResult.scoreChanged ? "#fde68a" : "#86efac"}`, borderRadius: 14, padding: 20, textAlign: "center" }}>
              <div style={{ fontSize: 48, fontWeight: 800, color: aiResult.pct >= 75 ? "#059669" : aiResult.pct >= 45 ? "#d97706" : "#dc2626", lineHeight: 1 }}>
            ...
            {aiResult.errorTopics.length > 0 && (
              <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: "14px 16px" }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#b91c1c", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>Errors found — will be added to your revision list</div>
